...
if(FALSE){
  # example call
  sf_name <- try(get_sf_name("sfBonferroni"), silent = TRUE)
  if (inherits(sf_name, "try-error")) { # Check if an error occurred
    sf_name <- "Unrecognized spending function"  # Assign default value if an error occurred
  }
}
